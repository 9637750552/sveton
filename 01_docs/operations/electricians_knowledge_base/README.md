...
Редакционная база знаний сейчас опирается на canonical statements после batch 012 по visual/OCR pass для `doc_012`:

- `590` валидных атомарных утверждений;
- `14` канонических кластеров покрывают все `590` утверждений;
...
- `0` extraction errors;
- `14` coverage / review warnings после batch 012: `3` по `doc_009`, `6` по `doc_001`, `5` по `doc_012`.
...
Сейчас в рабочем контуре закрыты четырнадцать исходных документов canonical-среза:
...
Для будущего раздела технических карт изделий canonical extraction и manual OCR pass по `img_0029` / `img_0030` уже собраны; editorial layer еще не собран, все операции изготовления кабеля, перемычек АКБ, подготовки проводов балансира, опрессовки, матриц и термоусадки остаются `review_required`, а остальные row-level изображения требуют отдельного visual-link review перед использованием в разделе.
