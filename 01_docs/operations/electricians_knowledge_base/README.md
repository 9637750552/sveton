...
Редакционная база знаний сейчас опирается на canonical statements после promotion-pass по `doc_008`:

- `513` валидных атомарных утверждений;
- `11` канонических кластеров покрывают все `513` утверждений;
...
- `0` extraction errors;
- `0` coverage / quality warnings по batch 008 и source coverage gate.
...
Сейчас в рабочем контуре закрыты одиннадцать исходных документов:
...
Для `06/11/13` сейчас нет отдельного канонического кластера. Это означает следующее:
