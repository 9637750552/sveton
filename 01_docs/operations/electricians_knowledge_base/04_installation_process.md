...
Статус: черновик редакционного раздела на базе canonical-утверждений. Технически критичные пункты требуют review перед использованием как финальной монтажной инструкции.
...
Изображения из `doc_015` пока не перенесены в editorial assets и не добавлены в `statement_images.jsonl`; связанные `image_id` указаны только как след extraction, а не как утвержденный визуальный слой.
...
- Связанные `image_id` из extraction: `26`.
...
- Нужен отдельный image-link pass для `doc_015`: перенести содержательные изображения в `assets/`, обновить `statement_images.jsonl` и заменить текстовые `image_id` на проверенный визуальный слой.
