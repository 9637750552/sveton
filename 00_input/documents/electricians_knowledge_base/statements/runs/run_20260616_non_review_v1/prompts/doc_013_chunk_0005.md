...

{
  ...
  "text": "**- Аккумуляторы (АКБ)**\n\nОбеспечивают запас энергии. Более подробное описание АКБ будет далее.",
  "related_image_ids": [],
  "excluded_image_ids": [],
  "previous_context": "**- Инвертор** Это преобразователь. Преобразует постоянный ток (DC) аккумуляторов в переменный (AC) для питания стандартных электропотребителей. Все инверторы, используемые в наших системах, имеют зарядные устройства, то есть при необходимости подзаряжают аккумуляторы.",
  "next_context": "**- Байпасный щит** Позволяет в ручном режиме переключать питание «От сети напрямую» или «Через ИБП». Более подробное описание байпасного щита будет далее.",
  "needs_review": false,
  "review_reasons": [],
